...
exports.getTokenDetail = async (req, res) => {
  ...
  var tokenType,
    tabs = [constants.TOKEN_PAGE_TABS.TRANSFERS];
  try {
    ...
    if (token && token.type) {
      ...
      if (tokenType === tokentype.erc20) {
        ...
        try {
          ...
          var sum = 0;
          const tokenHolders = await Account.find({
          ...
          for (let i = 0; i < tokenHolders.length; i++) {
            ...
            try {
              var balanceOf = await contract.methods
                .balanceOf(tokenHolders[i].address.toLowerCase())
                .call();
              balance = web3.utils.fromWei(balanceOf);
            } catch {
              balance = 0;
            }

            sum += balance[i];
            percent = (balance[i] / sum) * 100;

            var holders = {
              address: tokenHolders[i].address.toLowerCase(),
              balance: balance,
              percent: percent,
            };

            tokenHolder.push(holders);
          }
          tabs.push(
        ...
        var contract = new web3.eth.Contract(erc721ABI, tokenAddress);
...
const calcTokenBalance = (tokenTransfers, accountAddress) => {
  ...
  for (let i = 0; i < tokenTransfers.length; i++) {
    ...
    if (tokenTransfer.from === accountAddress)
